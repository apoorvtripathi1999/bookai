...
from flask import Flask, render_template, request, redirect, url_for, send_file, send_from_directory, fla
...
def upload_file():
    ...
    if 'audio_data' in request.files:
        ...
        txt_files = [f for f in os.listdir(app.config['UPLOAD_FOLDER']) if f.endswith('.txt')]
        txt_files.sort(reverse=True)  # newest first
        
        if txt_files:
            latest_txt_path = os.path.join(app.config['UPLOAD_FOLDER'], txt_files[0])
